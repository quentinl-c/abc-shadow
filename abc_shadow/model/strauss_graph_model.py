...
class StraussGraphModel(GraphModel):
    ...
    def evaluate(self, sample):
        ...
        """
        u = self.l0 * sample.get_edge_type_count(0)
        u += self.l1 * sample.get_edge_type_count(1)
        u += self.l2 * sample.get_edge_type_count(2)
        u += self.beta * sample.get_repulsion_count()
    ...
    def summary(results):
        data = dict()
        data['l0'] = [g.get_edge_type_count(0) for g in results]
        data['l1'] = [g.get_edge_type_count(1) for g in results]
        data['l2'] = [g.get_edge_type_count(2) for g in results]
        data['beta'] = [g.get_repulsion_count() for g in results]
